...
dependency_install() {
    ...
    judge "Compiler Toolkit Installation"
...
web_camouflage() {
    ## Please note that this conflicts with the default path of the LNMP script. Do not use this script in an environment where LNMP is installed, or you will be responsible for the consequences
    rm -rf /home/wwwroot
...
ssl_install() {
    if [[ "${ID}" == "centos" ]]; then
        ${INS} install shocked nc -y
    else
        ${INS} install shock netcat -y
    fi
...
domain_check() {
    ...
    echo -e "${OK} ${GreenBG} is getting public IP information, please wait patiently for ${Font}"
    ...
    if [[ -z ${local_ipv4} && -n ${local_ipv6} ]]; then
        ...
        echo -e "${OK} ${GreenBG} is recognized as IPv6 Only VPS, automatically add DNS64 server ${Font}"
    fi
    echo -e "The IP resolved by the DNS of the domain name: ${domain_ip}"
    echo -e "Local IPv4: ${local_ipv4}"
    echo -e "Local IPv6: ${local_ipv6}"
    sleep 2
    if [[ ${domain_ip} == ${local_ipv4} ]]; then
        echo -e "${OK} ${GreenBG} domain name DNS resolution IP matches local IPv4 ${Font}"
        sleep 2
    elif [[ ${domain_ip} == ${local_ipv6} ]]; then
        echo -e "${OK} ${GreenBG} domain name DNS resolution IP matches local IPv6 ${Font}"
        sleep 2
    else
        echo -e "${Error} ${RedBG} Please make sure the correct A / AAAA record is added to the domain name, otherwise V2ray ${Font} will not work normally"
...
old_config_exist_check() {
    if [[ -f $v2ray_qr_config_file ]]; then
        echo -e "${OK} ${GreenBG} detected the old configuration file, read the old file configuration [Y/N]? ${Font}"
        read -r ssl_delete
        case $ssl_delete in
        [yY][eE][sS] | [yY])
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "Please note, if you use Quantaumlt X / router / old version of Shadowrocket / V2ray core lower than version 4.18.1, please select compatibility mode"
...
ssl_update_manuel() {
    [ -f ${amce_sh_file} ] && "/root/.acme.sh"/acme.sh --cron --home "/root/.acme.sh" || echo -e "${RedBG} certificate signing tool Does not exist, please confirm whether you are using your own certificate ${Font}"
...
menu() {
    ...
    17)
        ...
        ;;
    18)
        read -rp "Please enter the camouflage path (note! No need to add slash eg:ray):" camouflage_path
        modify_camouflage_path
        start_process_systemd
        ;;
    *)
